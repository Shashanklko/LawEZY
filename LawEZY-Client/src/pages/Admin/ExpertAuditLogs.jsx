...
const ExpertAuditLogs = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const expRes = await apiClient.get(`/api/admin/experts/${id}`);
      setExpert(expRes.data);

      // Fetch logs
      const logsRes = await apiClient.get(`/api/admin/experts/${id}/logs`).catch(() => ({ data: [] }));
      setLogs(logsRes.data || []);
    } catch (err) {
      console.error("Audit data fetch failed:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredLogs = logs.filter(log => {
    ...
    // Sophisticated Financial Splitting
    if (filterType === 'REVENUE') {
      matchesFilter = log.type === 'FINANCE' && (log.amount > 0 || log.action.includes('PAYOUT') || log.action.includes('EARNING'));
    } else if (filterType === 'UTILIZATION') {
      matchesFilter = log.type === 'FINANCE' && (log.amount < 0 || log.action.includes('UTILIZE') || log.action.includes('SPENT'));
    }
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `AuditLog_${expert?.name || id}_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
  };
    ...
    <div className="admin-portal-wrapper">
      <aside className="admin-sidebar">
        ...
        <nav className="admin-nav">
          ...
          <button onClick={() => navigate(`/admin/experts/${id}/profile`)}>
            <span className="nav-icon">👤</span> Profile Dossier
          </button>
          <button className="active">
            <span className="nav-icon">📜</span> Expert Activity Logs
          </button>
        </nav>
        ...
        <header className="admin-top-bar">
          <div className="header-title">
            <div className="breadcrumb-dossier">
              <span onClick={() => navigate('/admin')}>Expert Management</span> / <span className="active">Activity Dossier</span>
            </div>
            <h1>Expert Audit Dossier: {expert?.firstName || expert?.name || 'Expert'}</h1>
            ...
            <div className="saas-dossier-toolbar">
              <div className="toolbar-left">
                ...
                <div className="saas-filter-tabs">
                  {['ALL', 'SESSION', 'REVENUE', 'UTILIZATION'].map(type => (
                    <button 
                      key={type}
...
export default ExpertAuditLogs;
